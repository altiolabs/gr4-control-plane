...
SchedulerCatalogService::SchedulerCatalogService(const catalog::SchedulerCatalogProvider& provider)
    : provider_(provider) {}
...
const SchedulerCatalogService::CachedCatalog& SchedulerCatalogService::cached_catalog() const {
    if (!cached_catalog_.has_value()) {
        ...
        std::sort(schedulers.begin(), schedulers.end(), [](const auto& left, const auto& right) {
